...
impl Default for BinOp {
    fn default() -> BinOp { BinOp::INCREMENT }
}
...
fn parse_line(line: &str) -> Instruction {
    ...
    for pair in pairs {
        ...
        match rule {
            Rule::register  => { instr.register = text; },
            Rule::increment => { instr.binop = BinOp::INCREMENT; },
            Rule::decrement => { instr.binop = BinOp::DECREMENT; },
            Rule::operand   => { instr.operand = text.parse().unwrap() },
            Rule::condreg   => { instr.cond.register = text; },
            Rule::eq        => { instr.cond.relop = RelOp::EQ; },
            Rule::neq       => { instr.cond.relop = RelOp::NEQ; },
            Rule::gte       => { instr.cond.relop = RelOp::GTE; },
            Rule::lte       => { instr.cond.relop = RelOp::LTE; },
            Rule::gt        => { instr.cond.relop = RelOp::GT; },
            Rule::lt        => { instr.cond.relop = RelOp::LT; },
